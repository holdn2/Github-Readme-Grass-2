import type { ContributionDay, GardenOptions } from "./types.js";
...
export function renderGardenSvg(days: ContributionDay[], options: GardenOptions): string {
...
function padCalendar(days: ContributionDay[], year: number): CalendarCell[] {
  const first = new Date(Date.UTC(year, 0, 1)).getUTCDay();
  const mondayOffset = (first + 6) % 7;
  return [...Array<CalendarCell>(mondayOffset).fill(null), ...days].slice(0, WEEKS * 7);
}

function renderMonthLabels(cells: CalendarCell[]): string {
  ...
  return cells
    .map((day, index) => {
      ...
      if (seen.has(month) || date.getUTCDate() > 7) return "";
      seen.add(month);
...
function renderWeekdayLabels(): string {
  return [
...
function renderCell(day: CalendarCell, index: number): string {
...
function renderTopTile(level: ContributionDay["level"], x: number, y: number, index: number): string {
...
function grassGrid(level: ContributionDay["level"], index: number): Array<{ x: number; y: number; size: number }> {
...
function renderFlowers(x: number, y: number, index: number): string {
  ...
  for (let i = 0; i < flowers; i += 1) {
    ...
    nodes.push(
      `<rect x="${fx}" y="${fy - 1}" width="1" height="1" fill="${petal}"/>`,
      `<rect x="${fx - 1}" y="${fy}" width="1" height="1" fill="${petal}"/>`,
      `<rect x="${fx}" y="${fy}" width="1" height="1" fill="#f0b43a"/>`,
      `<rect x="${fx + 1}" y="${fy}" width="1" height="1" fill="${petal}"/>`,
      `<rect x="${fx}" y="${fy + 1}" width="1" height="1" fill="${petal}"/>`,
    );
  }
...
function soilGrid(index: number): Array<{ x: number; y: number; size: number }> {
  const cells: Array<{ x: number; y: number; size: number }> = [];
...
function renderLegend(): string {
  ...
  return [
    ...
    [0, 1, 2, 3, 4]
      .map((level, index) => `<g transform="translate(${x + 34 + index * 17} ${y - 10}) scale(0.92)">${renderTopTile(level as ContributionDay["level"], 0, 0, index + 700)}</g>`)
      .join(""),
